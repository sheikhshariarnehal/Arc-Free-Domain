...
export async function POST(request: Request) {
  ...
  const { subdomain_id, type, content, name_prefix } = body
  ...
  const baseDomain = getFullDomain(subdomain.name)
  const cleanPrefix = name_prefix?.trim().replace(/\.+$/, '')
  const recordName = cleanPrefix ? `${cleanPrefix}.${baseDomain}` : baseDomain
